...
\begin{enumerate}
    \item The IMF is the function describing how many stars are born at each mass.
        ...
        \begin{enumerate}
            ...
            \begin{cases}
                ...
                % A = 0.158, M_c = 0.079, \sigma=0.69 & M\leq1 \msun\\
                \xi(\log M) = A M^{-\Gamma}   \\
                \Gamma = 1.3, A = 4.43\times10^{-2} & M>\msun
            \end{cases}
            $$
    ...
    In AST7939 Spring 2026,  you get credit for the Salpeter IMF, plus part \ref{sec:plot} for all three distributions.
    Completing the other distributions is extra credit, up to 20\% of the assignment total.
    Each step is worth 5 points.
    ...
    \begin{enumerate}
        ...
        \item \textbf{[5]} What is the average mass?
        \item \textbf{[5]} What is the average mass of stars with $M>8\msun$?
        \item \textbf{[5]} What is the ratio of the number of high-mass to low-mass stars?
            (use M=8 \msun as the dividing mass)
        \item \textbf{[5]} What is the ratio of the \emph{mass} of high-mass to low-mass stars?
        \item \textbf{[5]} (How) do these numbers change if you change $M_{max}$ to 100 \msun?  To 1000 \msun?
        \item \textbf{[5]} (How) do these numbers change if you change $M_{min}$ from 0.03 \msun to 0.3 \msun?
        \item \textbf{[5]} For a cluster of 1000 stars, how many would you expect to be
            $M>8\msun$ (able to go supernova)?
        \item \textbf{[10]} In an `optimal distribution function' (ODF), the cluster mass to maximum star mass
        ...
        \item \textbf{[5]} What \emph{mass} of cluster is required to produce a star of that mass?
        \item \textbf{[10]} In a probability distribution function, there is only a fixed likelihood
        ...
        \item \textbf{[5]} Compare the results from the ODF and the PDF for the presence of
            a $M>100\msun$ star.  How can you interpret the difference?
        \item \textbf{[5]} What is the expected light-to-mass ratio of each distribution
            assuming $L = \lsun \left(\frac{M}{\msun}\right)^{3}$?
        \item \textbf{[10]} The IMF is the \emph{system} IMF.  If we are primarily interested
        ...
        \item \textbf{[5]} For an ODF, the effective maximum stellar mass can be smaller.
