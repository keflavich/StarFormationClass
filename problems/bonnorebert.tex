...
\noindent\textbf{Problem Set: The Bonnor-Ebert Sphere}
...
In 2026, this is broken up into three assignments.  
This assignment can be solved entirely by LLMs, but using an LLM to do the homework is pointless - it knows how to solve this problem because the answer key is part of the training data (at least for some of the LLMs).
Therefore, a large fraction of the credit is in the `reflections' components.
You get credit for discussing what you learned, what was new vs review, and how and whether you might use what you learned.
If you think you learned nothing, you may say that, but in that case you need to compellingly argue why you learned nothing - and you'll be graded on how convincing the argument is.
...
\begin{enumerate}
\item \textbf{[5]} For the moment, assume that the gas density inside the sphere is uniform.
...
\item \textbf{[5]} Show that there is a maximum surface pressure $P_{\mathrm{s,max}}$ for
    which virial equilibrium is possible and derive its value.
    (find an extremum of $P_S(R)$ with respect to $R$)


\item \textbf{[5]} Now we will compute the true density structure. Consider first the equation of hydrostatic balance,
\begin{displaymath}
...
\item \textbf{[5]} Now consider the Poisson equation for the potential,
\begin{displaymath}
...
\item \textbf{[15]} Reflect.  Word count limit $<250$ words, emphasis on $<$.

\item \textbf{[25]} Numerically integrate the isothermal Lane-Emden equation subject to the
...
\item {[10]} The total mass enclosed out to a radius $R$ is
...
%     (use plots in your evaluation)
 ...
 \item {[5]} B68 is a real Bonnor-Ebert sphere.  It has a measured shape parameter $\xi_{max}=6.9$.
     Is the core stable?  Recall that in part \ref{item:maxmass}, you determined the maximum stable dimensionless
     mass.

 \item {[5]} B68 has an outer radius of 150" at its distance of 128 pc.  Its average temperature
     is $T=10.5$ K.  What is its central density $\rho_c$?  Recall Part \ref{item:r0} and the definition $\xi=r/r_0$ (so $\xi_{max} = r_{max}/r_0$).

 \item {[5]} Compare the surface pressure of B68 to the typical ISM pressure $P/k = 2\times10^4 \mathrm{cm}^{-3} \mathrm{K}$.
     Recall from part \ref{item:totalmass} that $$\frac{P}{P_c} = \frac{\rho}{\rho_c} = e^{-\psi}$$
    and the ideal gas law ($P = \rho k_B T$).

 \item {[15]} Discuss what you have derived and learned over the full problem set.  
\end{enumerate}
