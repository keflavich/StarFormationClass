...
\begin{enumerate}
    \item The mass fraction of hydrogen, helium, and metals is \citep{Kauffmann2008}
        ...
        You may assume any reasonable value for $M(Z)$. % <- this is basically misdirection
        % see Kauffmann+ 2008 MAMBO appendix
        % mean mass per hydrogen atom is \mathcal{M} / M(H) = 1 / 0.71
        % mean mass per hydrogen molecule is \mathcal{M} / M(H2) = \mathcal{M} / (2 M(H)) = 2 / 0.71 = 2.82
        % mean mass per free particle in atomic gas is \mathcal{M} / (N(H2) + N(He) + N(z)) = 1/( 0.71 / 1 + 0.27 / 4 + 0.02 / 50) = 1.286
        % mean mass per free particle is \mathcal{M} / (N(H2) + N(He) + N(z)) = 1/( 0.71 / 2 + 0.27 / 4 + 0.02 / 16) = 2.359
    ...
    \item Compute: What is the free-fall timescale in molecular clouds?  
        ...
        % tff = np.sqrt(3*np.pi/(32*constants.G*(1e2*u.cm**-3*2.8*u.Da))).to(u.Myr)
        % 3.08 Myr
    ...
    \item Compute: What is the crossing time of a typical molecular cloud?  The
        ...
        % c_s = np.sqrt(constants.k_B * 20*u.K / (2.34 * u.Da)).to(u.km/u.s)
        % t_c = (10*u.pc / c_s).to(u.Myr)
        % c_s, t_c
        % 0.266 km/s, 36.6 Myr
    ...
    \item If the molecular cloud forms stars at 100\% efficiency (all of the
        ...
        % M = ((100*u.cm**-3 * 2.8*u.Da) * (4/3.*np.pi*(10*u.pc)**3)).to(u.M_sun)
        % SFR = M / tff = 0.009 Msun/yr
...
\item \textbf{Molecular Cloud Kinematics} \\
    ...
    \begin{enumerate}
        \item (G) Typical clouds in the Solar Neighborhood have line widths $FWHM\approx 1.5 \mathrm{km~s}^{-1}$ on $\sim1$ pc size scales.
            ...
            % results in 0.14 pc.... but actually 0.37 km/s??
